...
const cardList = (containerId) => {
  return taskman.cards.filter((card) => card.id_container === containerId)
}
...
const dropItem = (event, containerId) => {
  ...
  const item = taskman.cards.find((card) => card.id == id)
  item.id_container = containerId
  taskman.last_modified = new Date().toLocaleString('en-GB')
  store.dispatch('taskman/settaskman', taskman)
}
...
const deleteItem = (type) => {
  if (type === 'container') {
    ...
    taskman.cards = taskman.cards.filter(
...
const handleKanbanAction = (mode, type, containerId, payload) => {
  if (mode === 'add') {
    if (type === 'container') {
      ...
      newCardData.id =
        taskman.cards.length > 0 ? [...taskman.cards].pop().id + 1 : 1
...
const handleEditCard = (type, selectedCard) => {
  if (type === 'change') {
    ...
    state.tempCards = taskman.cards.map((card) => ({ ...card }))
  } else if (type === 'save') {
...
<template>
  <div
    class="flex h-full w-full overflow-auto rounded-lg bg-[#F2F3F9] bg-opacity-10 backdrop-blur-lg px-2 py-3"
  >
    <TransitionGroup name="list">
      <div
        v-for="container in taskman.containers"
        :key="container.id"
        class="mx-1"
      >
        <div
